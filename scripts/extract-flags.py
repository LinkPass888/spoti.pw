#!/usr/bin/env python3
"""Write tweak/src/SGFlagList.m, the table of Spotify's remote-config flags, from a decrypted IPA.
...
"""
import re, struct, subprocess, sys, tempfile, zipfile
...
def main():
    ...
    version = re.search(r'(\d+\.\d+\.\d+)', ipa.name).group(1)
    lines = [f'// Generated by scripts/extract-flags.py from Spotify {version}. Do not edit.',
             '#import "SGCommon.h"', '', 'const SGFlagDef SGFlagTable[] = {']
    ...
    out = ROOT / 'tweak/src/SGFlagList.m'
    out.write_text('\n'.join(lines))
